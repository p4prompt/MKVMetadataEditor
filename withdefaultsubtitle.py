...
def delete_file(file_path):
    ...
    try:
        if os.path.exists(file_path):
            print(f"Deleting temporary subtitle file {file_path}...")
            os.remove(file_path)
    except Exception as e:
...
def add_subtitle_as_latest_track(input_file, output_file, subtitle_file):
    ...
    try:
        ...
        # Correct mkvmerge command to add subtitle track
        subprocess.run(
            [
...
def set_mkv_metadata_for_folder(input_folder, output_folder):
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".mkv"):
            ...
            try:
                print(f"Setting global title to 'Visit & Support - Google.com' for {output_file}")
                subprocess.run(
                    ['mkvpropedit', output_file, '--edit', 'info', '--set', 'title=Visit & Support - Google.com'],
                    check=True
                )
            except subprocess.CalledProcessError as e:
            ...
            for track_number, track_type in tracks:
                ...
                try:
                    if track_type == "video":
                        print(f"Setting title for video track {track_number} to 'Google.com' in {output_file}")
                        subprocess.run(
                            ['mkvpropedit', output_file, '--edit', f"track:{track_number}", '--set', 'name=Google.com'],
                            check=True
                        )
                    elif track_type == "audio":
                        print(f"Setting title for audio track {track_number} to 'Google.com' in {output_file}")
                        subprocess.run(
                            ['mkvpropedit', output_file, '--edit', f"track:{track_number}", '--set', 'name=Google.com'],
                            check=True
                        )
                    elif track_type == "subtitles":
                        print(f"Setting title for subtitle track {track_number} to 'Google.com' in {output_file}")
                        subprocess.run(
                            ['mkvpropedit', output_file, '--edit', f"track:{track_number}", '--set', 'name=Google.com'],
                            check=True
                        )
